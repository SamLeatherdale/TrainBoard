...
import { defaultIgnore, FocusableItem, Metrics } from "./_focusable-item";

// const FOCUSABLE_ITEM_SELECTOR = ".dpad-focusable";
const FOCUSABLE_ITEM_SELECTOR = "[tabindex]";
...
export class DpadController {
    ...
    findFocusableItems() {
        const focusableItems: NodeListOf<HTMLElement> =
            document.querySelectorAll(FOCUSABLE_ITEM_SELECTOR);
    ...
    setCurrentFocusItemByElement(el: HTMLElement) {
    ...
    private updateNeighbors(fi: FocusableItem) {
        ...
        for (let i = 0; i < itemCount; i++) {
            ...
            if (
                distanceBottom !== null &&
                (typeof minBottomElementDist === "undefined" ||
    ...
    private onKeyDown(event: KeyboardEvent) {
        const ignore = this.currentlyFocusedItem?.getIgnore() || defaultIgnore;
        if (event.keyCode === KeyCode.TAB) {
            // Tab
        } else if (event.keyCode === KeyCode.LEFT && !ignore.left) {
            // Left
            event.preventDefault();
            this.moveFocus({ x: -1, y: 0 });
        } else if (event.keyCode === KeyCode.UP && !ignore.up) {
            // Up
            event.preventDefault();
            this.moveFocus({ x: 0, y: 1 });
        } else if (event.keyCode === KeyCode.RIGHT && !ignore.right) {
            // Right
            event.preventDefault();
            this.moveFocus({ x: 1, y: 0 });
        } else if (event.keyCode === KeyCode.DOWN && !ignore.down) {
            // Down
            event.preventDefault();
            this.moveFocus({ x: 0, y: -1 });
        }
